...
def train_generator(Htrain):
    ...
    YP00 = np.concatenate((signal_output00[0:K],signal_output00[K:2*K]))
    YP01 = np.concatenate((signal_output01[0:K], signal_output01[K:2 * K]))
    YP10 = np.concatenate((signal_output10[0:K], signal_output10[K:2 * K]))
    YP11 = np.concatenate((signal_output11[0:K], signal_output11[K:2 * K]))
    ...
    label00 = np.concatenate(
        (np.real(H_label[0, :]), np.imag(H_label[0, :])))
    label01 = np.concatenate(
        (np.real(H_label[1, :]), np.imag(H_label[1, :])))
    label10 = np.concatenate(
        (np.real(H_label[2, :]), np.imag(H_label[2, :])))
    label11 = np.concatenate(
        (np.real(H_label[3, :]), np.imag(H_label[3, :])))
